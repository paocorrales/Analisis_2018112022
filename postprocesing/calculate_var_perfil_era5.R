...
for (d in 1:length(dates)) {
  ...
  era_file <- paste0(era_path, "ERA5/met_em.d01.", format(dates[d], "%Y-%m-%d_%H:%M:%S"), ".nc")
  ...
  ana_file <- Sys.glob(paste0(exp_path, exp, "/", toupper(run), "/", format(dates[d], "%Y%m%d%H%M%S"), "/*.ensmean"))
  ...
  out <- era %>% 
    melt(id = c("date", "south_north", "west_east", "lev"), value.name = "era") %>% 
    .[ana %>% melt(id = c("date", "south_north", "west_east", "lev"), value.name = run), on = .NATURAL] %>% 
    .[, .(rmse = sd(get(run) - era, na.rm = TRUE),
          bias = mean(get(run) - era, na.rm = TRUE),
          ana = mean(get(run), na.rm = TRUE),
...
fwrite(out_ana, paste0("~/datosmunin3/EXP/derived_data/perfiles_", run, "_era5_", exp, ".csv"))
