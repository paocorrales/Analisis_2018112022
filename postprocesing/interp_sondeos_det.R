...
files <- list.files(path = paste0("/glade/scratch/jruiz/EXP/", args[1], "/FCST/det", args[2],"/"), 
...
for (f in files) {
  ...
  descriptores <- unglue::unglue(f, c("/glade/scratch/jruiz/EXP/{exp}/FCST/det{fcst}/wrfout_d01_{date}.mean", "/glade/scratch/jruiz/EXP/{exp}/FCST/det{fcst}/wrfout_d01_{date}"))
...
# Leo pronóstico con algo de post procesamiento
  ...
  fwrite(subset, paste0("/glade/scratch/jruiz/EXP/analisis/sondeos/det", args[2], "/sondeo_", descriptores[[1]][["exp"]], "_",
                         "det",  format(fcst_time, "%Y%m%d%H%M%S"), ".csv"))
} 
