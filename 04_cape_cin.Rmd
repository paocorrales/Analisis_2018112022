...
ggsave("fig/cape_hov.png", dpi = 150, width = unit(20, "cm"), height = unit(10, "cm"))
```
...
ggsave("fig/cape_esp.png", dpi = 150, width = unit(20, "cm"), height = unit(15, "cm"))
```
...
perfil_lev_E4[perfil_lev_E3, on = c("lev", "date")] %>%
  ...
  labs(title = "Temperature difference (K)",
       ...
       x = "Time",
       y = "Pressure level") +
  scale_y_level(name = "Pressure level") +
  scale_x_datetime(expand = c(0,0), date_labels = "%b-%d %H:%M", date_breaks = "12 hours") +
  theme_minimal(base_size = 26) +
  theme(legend.position = "bottom",
        panel.ontop = TRUE,
        panel.grid = element_line(linetype = 3))

ggsave("fig/temperature_diff.png", dpi = 150, width = unit(15, "cm"), height = unit(10, "cm"))
...
ggsave("fig/humidity_diff.png", dpi = 150, width = unit(15, "cm"), height = unit(10, "cm"))
```
...
temp_spd[bottom_top <= 7, .(t = mean(t), u = u[bottom_top == 1], v = v[bottom_top == 1]), 
         by = .(south_north, west_east, date, exp)] %>% 
  .[coord, on = c("south_north", "west_east")] %>% 
  # .[, c("u", "v") := temp_spd[bottom_top == 1, .(u, v)]] %>%
  .[, date := factor(date)] %>% 
  ggplot(aes(lon, lat)) +
