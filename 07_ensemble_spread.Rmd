...
```{r include=FALSE}
path <- "/home/paola.corrales/datosmunin/EXP/E8/ANA/*/diagfiles/asim_conv_2018112*"

perfiles <- read_diag(path, variable = c("t")) %>%
...
perfiles[type %in% c(181, 187, 281, 287)] %>%
  .[, var := fifelse(var == "u", "v", var)] %>% 
  # .[]
  mesoda::get_RCRV(tipo.var = "superficie", calculo = c("box")) %>% 
  fwrite("rcrv_V_box.csv")


perfiles[type %in% c(231, 233, 220)] %>%
  .[, var := fifelse(var == "u", "v", var)] %>% 
  .[, error.level := nivel.error] %>% 
  mesoda::get_RCRV(tipo.var = "perfil", calculo = c("unico")) %>%
  fwrite("rcrv_V_perfil.csv")


perfiles[type %in% c(181, 187)] %>%
  .[, var := fifelse(var == "u", "v", var)] %>% 
  # .[]
  mesoda::get_RCRV(tipo.var = "superficie", calculo = c("box")) %>% 
  fwrite("rcrv_t_box.csv")


perfiles[type %in% c(131, 133, 120)] %>%
  .[, var := fifelse(var == "u", "v", var)] %>% 
  .[, error.level := nivel.error] %>% 
  mesoda::get_RCRV(tipo.var = "perfil", calculo = c("unico")) %>%
  fwrite("rcrv_t_perfil.csv")
