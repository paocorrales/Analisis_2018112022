...
```{r echo=FALSE}
fread("uvt_superficie_temporal.csv") %>% 
  .[, date := ymd_hms(date)] %>% 
  ...
  labs(title = "Consistency ratio for surface observations",
       x = "hour",
       color = "Type of obs") +
  theme_minimal()

fread("uvt_superficie.csv") %>%
  .[, .(var, type, count, cr)] %>% 
  knitr::kable()
```
...
fread("uvt_perfil.csv") %>% 
  ...
  labs(title = "Consistency ratio for sounding and aircraft observations",
       x = "nivel",
       color = "Type of obs") +
  theme_minimal()
```
