...
files <- Sys.glob("/home/paola.corrales/datosmunin/EXP/E[7-8]/ANA/*/satbias_enkf/satbias_2*")
...
for (this_sensor in sensores){
  ...
  g <- satbias[sensor %in% this_sensor & iuse == 1] %>% 
    ...
    scale_color_brewer(type = "qual", palette = "Dark2",
                       labels = c("coeff1" = "Offset",
                                  ...
                                  "coeff5" = "TLR",
                                  "coeff8" = "Emiss")) +
    scale_linetype_manual(values = c(2, 1), labels = c("RAD", "NEW RAD")) +
  ...
  if (h > 0){
    ...
    ggsave(paste0("/home/paola.corrales/campos/", this_sensor, ".png"), plot = g, width = 9, height = h, limitsize = FALSE)
  }
}
```
...
```{r}
files <- Sys.glob("/home/paola.corrales/datosmunin/EXP/E[7-8]/ANA/*/diagfiles/asim*ensmean")

files <- files[!str_detect(files, pattern = "conv")]

diag_rad <- purrr::map(files, function(f) {
  
  meta <- unglue(f, "/home/paola.corrales/datosmunin/EXP/{exp}/ANA/{date}/diagfiles/{file}.ensmean")
  
  out <- mesoda::read_diag_rad(f, exp = meta[[1]][["exp"]]) %>%
    use_table[., on = c("sensor", "channel")] %>% 
    .[, usage.flag := fifelse(errinv %between% c(0.0000316227, 1000) & 
                                iuse == 1 &
                                qc == 0 & 
                                peakwt %between% c(0.001, 1200), 1, -1)] %>% 
    mutate(across(channel:pred12, as.numeric))
}) %>% 
  rbindlist() 
...
for (s in sensores) {
  ...
  diag_rad %>% 
    .[sensor == s & usage.flag == 1, .(tbc_mean = mean(tbc, na.rm = TRUE),
        tbcnob_mean = mean(tbcnob, na.rm = TRUE),
        tbc_sd = sd(tbc, na.rm = TRUE),
        tbcnob_sd = sd(tbcnob, na.rm = TRUE),
        count = .N),
    by = .(sensor, channel, exp)] %>% 
    melt(id.vars = c("sensor", "channel", "exp"), 
         measure.vars = c("tbcnob_mean", "tbc_mean", "tbc_sd", "tbcnob_sd")) %>% 
    separate(variable, into = c("variable", "estadistico")) %>% 
    dcast(sensor + channel + exp + variable ~ estadistico, value.var = "value") %>% 
    ggplot(aes(channel, mean)) +
    geom_hline(yintercept = 0, color = "grey20") +
    # geom_errorbar(data = ~.x[run == "training" & variable == "tbcnob"],
    #               aes(ymin = mean - sd, ymax = mean + sd, fill = "no BC"), width = 0.15, position = pd) +
    # geom_point(data = ~.x[run == "training" & variable == "tbcnob"], aes(fill = "no BC"),
    #            size = 2, shape = 15, position = pd) +
    geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd, color = exp, alpha = variable,
                      group = interaction(exp, variable)), 
                  width = 0.15, position = pd) +
    geom_point(aes(color = exp, group = interaction(exp, variable),  alpha = variable), position = pd) +
    scale_color_manual(values = c("darkorange", "cyan4"),  labels = c("RAD", "NEW RAD")) +
    scale_alpha_manual(values = c("tcb" = 1, "tbcnob" = 0.4), labels = c("corregido", "original")) +
    labs(x = "Channel", y = "Mean BIAS", color = NULL, alpha = NULL,
         caption = paste0(s)) +
    theme_minimal()
  
  ggsave(paste0("/home/paola.corrales/campos/mean_bias_E8_", s, ".png"), width = 10, height = 6)
}
...
```{r}
diag_rad[, lat2 := as.numeric(diag_rad$lon)] %>% 
  .[is.na(lat2), c("sensor", "date", "id")]
