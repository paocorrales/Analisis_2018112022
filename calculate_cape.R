...
for (f in 1:length(files)) {
  ...
  date <- lubridate::ymd_hms(str_extract(path, "\\d{14}"))
  ...
  cape <- ana[, capear(P, T, QVAPOR), by = .(south_north, west_east, date)]
...
fwrite(out, "/glade/scratch/jruiz/EXP/analisis/cape_cin_ana_E3.csv")
