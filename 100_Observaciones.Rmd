...
```{bash, eval=FALSE, echo=TRUE}
    - dpres      = 4.6051701859880918 --> data(ipres,i)=pressure
...
Si bien `drpx=3.3243794141870162` luego del if de más arriba, luego en el código y antes de calcular el `rario_errors` chequea lo siguiente:
...
Parece que el valor de `dpres` cambia cuando corre una rutina que ¿traduce? a puntos de retícula: `grdcrd1()` (según la rutina: get grid coordinates from monotonically increasing or decreasing points). Al salir de esta rutina `dpres=-8.5960447079269979`. 
