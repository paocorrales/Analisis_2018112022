...
BC_Constant: constant bias correction term
BC_Scan_Angle: scan angle bias correction term
BC_Cloud_Liquid_Water: CLW bias correction term
BC_Lapse_Rate_Squared: square lapse rate bias correction term
BC_Lapse_Rate: lapse rate bias correction term
BC_Cosine_Latitude_times_Node: node*cos(lat) bias correction term
BC_Sine_Latitude: sin(lat) bias correction term
BC_Emissivity: emissivity sensitivity bias correction term
BC_Fixed_Scan_Position: external scan angle
...
diag[sensor == "amsua_n15" & channel == 2] %>% 
  .[date == date[1]] %>% 
  melt(measure.vars = paste0("pred", 8)) %>% 
  ggplot(aes(ConvertLongitude(lon), lat)) +
