...
perfiles %>% 
  ggplot(aes(date, lev)) +
  geom_contour_fill(aes(z = Q_E7_E4*1000), breaks = seq(-0.0004, 0.0016, 0.0002)*1000) +
  scale_fill_divergent(name = NULL,
...
perfiles  %>% 
  ggplot(aes(date, lev)) +
  geom_contour_fill(aes(z = V_E7_E4), breaks = seq(-1, 1, 0.2)) +
  scale_fill_divergent(name = NULL,
                       breaks = seq(-1, 1, 0.2),
                       guide = guide_colorstrip(inside = TRUE,
                                                barwidth = 25,
...
q[bottom_top <= 7, .(q = mean(q), u = u[bottom_top == 1], v = v[bottom_top == 1]), 
  ...
  facet_grid(date ~ exp, 
             labeller = labeller(exp = c(E4 = "CONV", E5 = "AUT", E6 = "SATWND", E7 = "RAD"))) +
...
diff_q_spd %>% 
  melt(measure.vars = c("diff_E6_E5", "diff_E7_E4"), variable.name = "diff") %>%
