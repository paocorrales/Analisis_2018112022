...
t <- lapply(seq_len(nrow(fcsts)), function(f) {
  ...
  %>% 
  .[, date := factor(date)] %>%
  melt(id.vars = c("lon", "lat", "date", "exp", "bottom_top"), .[coord, on = c("south_north", "west_east")] 
       measure.vars = c("t", "u", "v")) %>% 
  dcast(lon + lat + date + variable + bottom_top ~ exp, value.var = "value") %>% 
    .[, diff_E7_E4 := E7 - E4]

t[bottom_top %in% seq(1, 28, 3) & variable == "t" & date == unique(date)[2]] %>% 
  .[, date := factor(date)] %>% 
  ggplot(aes(lon, lat)) +
...
```{r}
update <- ReadNetCDF("analisis/u_ana_20181123000000_E7.nc", 
           vars = c(ana = "uvmet")) %>% 
    .[, guess := ReadNetCDF("analisis/u_guess_20181123000000_E7.nc", 
...
diag[var != "t" & type %in% c(181, 287), .N, by = .(exp, type, var)] %>% 
  dcast(type + var ~ exp)
```
